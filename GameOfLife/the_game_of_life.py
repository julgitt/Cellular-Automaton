...
class TheGameOfLife:
    ...
    def set_plot_properties(self):
        ...
        self.set_plot_title()
        self.set_plot_game_tutorial()
        
        
    def set_plot_title(self):
        title_text = f'Current mode: {self.current_mode.name}\n' \
                     + ('The game has started' if self.game_is_running else 'Press space to start the game')
        plt.title(title_text)


    def set_plot_game_tutorial(self):
        xlabel_text = \
            '''Change interval to:  \n''' + \
            '''     500ms:  1       \n''' + \
            '''     100ms:  2       \n''' + \
        ...
        plt.xlabel(xlabel_text, loc = 'left')
